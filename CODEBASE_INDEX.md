...
| Método | Descripción |
|---|---|
| `LoadAsync()` | Carga el `.ini` leyendo con `FileStream`/`FileShare.ReadWrite \| FileShare.Delete` y reintentos con backoff (`LeerTodasLasLineasConReintentoAsync`) para tolerar locks temporales (antivirus, emuMMC montada, otro proceso) |
...
| `SaveAsync()` | Persiste el `.ini` escribiendo con `FileShare.Read` y reintentos (`EscribirTextoConReintentoAsync`) para evitar `IOException` por archivo bloqueado |
...
| `ParticionadorDiscos.cs` | `ParticionadorDiscos` | `ParticionarYFormatearAsync(...)`, `ParticionarSimpleYFormatearAsync(...)`, `FormatearSoloFAT32Async(...)`, `FormatearYEtiquetarAsync(...)`. **Fix particionado:** se eliminó el `format fs=fat32 ... noerr` de diskpart en la Fase 2 (rompía la creación de la partición emuMMC dejándola "No asignado"/RAW); ahora se verifica explícitamente con `ContarParticionesDiscoAsync(numeroDisco, ct)` que existan al menos 2 particiones reales tras particionar. **Vigilante de diálogos:** `IniciarVigilanteDialogosError()` corre en segundo plano durante particionado/formateo y cierra diálogos nativos de Windows (p. ej. "Ubicación no disponible", volumen sin sistema de archivos reconocido) vía `CazadorVentanas.CerrarDialogosDeError()` |
...
|---|---|---|
| `MainWindow.xaml.cs` | Inicialización | `MainWindow()` — constructor, suscripción a `_cerebro.GistActualizadoEnBackground`; `AplicarConfiguracionRemota(GistData)` — vuelca config UI, re-fusiona `MasterKeyTable`/`ModeloSwitchTable` y mapea `datos.Tools` a `ConfiguracionRemota.Tools` (sección raíz `"tools"` del Gist), llamado en carga inicial y en cada re-sincronización; `OnGistActualizadoEnBackground(GistData)` — llamado en hilo UI cuando el Gist cambia en background, repuebla modelo/región/llaves del panel derecho sin reiniciar; `ConfigurarEventos()` suscribe `_notificadorDiscos.UnidadDesconectada` para llamar además a `CerrarDialogosSistemaSDConReintentoAsync()`, que cierra diálogos nativos de Windows que pudieran quedar en primer plano tras retirar la microSD (usa `_ultimaLetraSdConocida` de `MainWindow.SD.cs` vía `CazadorVentanas.CerrarDialogosDeError()`) |
| `MainWindow.SD.cs` | Unidades SD | `HayMicroSdConectada()` — guard común usado por los flujos de instalación (asistido parcial/completo, instalación desde PC, formato, particionado) para bloquear la operación si no hay microSD seleccionada; `_ultimaLetraSdConocida` — última letra de SD vista conectada, usada para cerrar diálogos nativos de Windows tras una desconexión (el combo ya no la contiene en ese momento); `RefrescarVersionAtmos()`; `RefrescarPanelInfoSD()` — refresca todos los campos del panel derecho (capacidad, formato, serial, modelo/región, sección LLAVES) sin red ni reinserción de SD — llamado tras restaurar llaves; `OcultarSeccionLlaves()`; `MostrarSeccionLlaves(info)` — muestra la sección LLAVES del panel con datos de `InfoPanelDerecho`; re-aplica `AplicarConfiguracionRemota` y repuebla modelo/región/llaves tras cada re-sincronización del Gist; **Firmware emuMMC (async, no bloqueante):** `IniciarDeteccionFirmwareEmummcAsync(info)` — cancela cualquier detección previa (`CancelarDeteccionFirmwareEmummc()`), muestra "Detectando firmware de emuMMC..." y lanza `EjecutarDeteccionFirmwareEmummcAsync` sin bloquear el resto del panel (ya pintado por `ObtenerInfoPanel`, síncrono). Usa un id de operación incremental + comparación de `DiscoFisico` para descartar resultados obsoletos si la unidad cambió mientras se detectaba. `CancelarDeteccionFirmwareEmummc()` también se invoca desde `LimpiarInterfazSD()` y `MainWindow.Ventana.cs` (`BtnCerrar_Click`/`BtnClose_Click`) |
| `MainWindow.InstalacionDesdePc.cs` | Instalación desde carpeta local de PC | `AbrirOverlayInstalacionDesdePc()` — bloquea sin microSD (`HayMicroSdConectada()`), pide carpeta con `OpenFolderDialog`, valida con `EsPaqueteAtmosphereValido(carpetaRaiz)` que exista `atmosphere`, `bootloader` o `switch` en la raíz (si no, muestra aviso de "carpeta no contiene un paquete de Atmosphere válido"), copia recursivamente el contenido a la raíz de la SD vía `PipelineFsHelpers.CopiarDirectorio(...)` mostrando progreso (`EstadoProgreso`/`ControladorCarga`); `EsPaqueteAtmosphereValido(string carpetaRaiz)` — valida estructura mínima de un paquete Atmosphere |
...
| `VistaAsistida` | `VistaAsistida.xaml(.cs)` | Vista del asistente. Eventos: `InstalacionSolicitada`, `DetalleModuloSolicitado`, `ProcesarCompletoSolicitado`, **`VolverAlHubSolicitado`** (botón «VOLVER A CFW» en `OverlaySelectorModo`, manejado en `MainWindow.Navegacion.cs` ? `MostrarVistaHubCFWDesdeAsistido()`). Método: `Cargar(nodos, modulos, modoAsistente)`. El banner de versión compatible (`ConfiguracionRemota.Ui.VersionCompatible`) fue **migrado a la tarjeta INSTALACION de `VistaHubCFW`** — `OverlaySelectorModo` ya no lo muestra. Selector de modo en `OverlaySelectorModo`: `ModoCompleto_Click`; `ModoParcial_Click` — bloquea si no hay microSD (`MainWindow.HayMicroSdConectada()`); `ModoDesdePc_Click` — ya no es placeholder, bloquea sin microSD y llama a `MainWindow.AbrirOverlayInstalacionDesdePc()`; `DescargarEnPc_Click` — abre el overlay de descarga local (`MainWindow.Actualizacion.cs`, carpeta destino por defecto `ATMOS-{version}`); `BtnVolverAlSelector_Click`, `BtnVolverAlHubCFW_Click` |
...
| `ProcesarCompletoArgs : EventArgs` | Args del evento de proceso completo: `GbEmuMMC`, `LetraSD`, `Etiqueta`, `NumeroDisco`, `Modulos`, `IdsDependencias`, `Logger`, `SoloInstalar`, `EsDescargaLocal` (distingue descarga local vs instalación sobre SD) |
...
| `EstilosBotones.xaml` | Estilos de botones. **`EstiloBotonVolver`**: pill simple con borde `AcentoCian` al hover (usado en Detalle de módulo). **`EstiloBotonVolverPremium`**: botón "VOLVER" reutilizable en todas las vistas del hub CFW (Instalación/Actualización — `BtnVolverAsistidoCompleto` en `MainWindow.xaml`; Catálogo/Personalización — `BtnVolverHubDesdeCatalogo` en la cabecera de búsqueda; Alertas y Estado — botón en `VistaDiagnosticoSD`; selector de modo de `VistaAsistida` — `BtnVolverAlHubCFW`). Mismo look "pill" que `EstiloBotonVolver` pero con halo neon rotatorio (`PincelNeonRotatorio`) + `DropShadowEffect` glow + escala sutil (`ScaleTransform` 1.04) al hover, coherente con el hover premium de las tarjetas del hub (`VistaHubCFW.EstiloHaloHub`). El contenido (icono `Path` + texto) lo define cada caller vía `ContentPresenter`. |
...
*Actualizado: 2025 — rama `fix(Instalacion)` — UX del hub CFW e Instalación/Actualización: (1) el banner de versión compatible (`ConfiguracionRemota.Ui.VersionCompatible`) se movió de `VistaAsistida.OverlaySelectorModo` a la tarjeta **INSTALACION** de `VistaHubCFW`; (2) `PanelAsistidoCompletoOverlay` eliminó el selector de modo intercambiable (`SelectorModoAsistido`/`BtnModoCompleto`/`BtnModoSoloInstalar`) — ahora cada punto de entrada del hub abre el overlay en un modo fijo: "Instalación" (tarjeta hub) siempre `INSTALACION COMPLETA` (formatea + instala), "Actualización" siempre `ACTUALIZACION`/Solo Instalar (sin formatear/particionar); (3) `VistaAsistida.OverlaySelectorModo` ganó un botón «VOLVER A CFW» (evento `VolverAlHubSolicitado` ? `MainWindow.MostrarVistaHubCFWDesdeAsistido()`) para regresar al dashboard del hub sin cerrar la app; (4) **botón "VOLVER" premium unificado** (`EstiloBotonVolverPremium` en `EstilosBotones.xaml`) aplicado en las 4 vistas del hub — Instalación/Actualización (`BtnVolverAsistidoCompleto`), Catálogo/Personalización (`BtnVolverHubDesdeCatalogo`, nuevo en la cabecera de búsqueda de `MainWindow.xaml` + handler en `MainWindow.Navegacion.cs`), Alertas y Estado (botón existente en `VistaDiagnosticoSD`) y el selector de modo de `VistaAsistida` (`BtnVolverAlHubCFW`) — mismo look "pill" que el `EstiloBotonVolver` original pero con halo neon rotatorio + glow + escala al hover, replicando el lenguaje visual premium de las tarjetas del hub.*

*Actualizado: 2025 — rama `fix(Instalacion)` — corrección de scope del overlay `PanelSelectorActualizarOverlay` (selector "Elige cómo quieres actualizar", tarjetas "Actualizar paquete predefinido" / "Actualizar Módulos"): estaba definido como overlay de ventana completa (`Grid.Row="0" Grid.RowSpan="2" Grid.Column="0" Grid.ColumnSpan="3"`), lo que cubría indebidamente la TopBar y los paneles laterales. Se movió a `Grid.Row="1" Grid.Column="1"` (misma celda que `GridContenidoCentral`/`VistaHubCFW`/`VistaAsistida`/`VistaCatalogo`), dejando la TopBar y los paneles izquierdo/derecho visibles e interactivos — mismo comportamiento que `VistaAsistida.OverlaySelectorModo` (que respeta estas zonas por estar embebido en un `UserControl` que solo vive en esa celda). En `MainWindow.Navegacion.cs`, `AbrirSelectorActualizacion()`/`CerrarSelectorActualizacion()` dejaron de usar `MostrarOverlayConAnimacion`/`AplicarBlurFondo` (que difuminan toda la ventana incluida la TopBar) y ahora solo animan la opacidad del propio overlay in/out. Los demás overlays de ventana completa (`PanelActualizacionOverlay`, `PanelQueueOverlay`, `PanelLogOverlay`, `PanelRp2040Overlay`, `PanelRespaldoLlavesOverlay`, `PanelLimpiezaSDOverlay`, `PanelAjustesOverlay`, `PanelFormatoFAT32Overlay`, `PanelParticionadoOverlay`, `PanelAsistidoCompletoOverlay`, `PanelDependenciasOverlay`, `PanelPersonalizacionBackdrop`) siguen siendo full-window a propósito — son modales de app genuinos, no vistas de navegación del hub CFW.*

*Actualizado: 2025 — rama `feat(incompatibilidades)` — nueva **Fuente D** de detección en el escáner de "Alertas y Estado" (`MainWindow.Diagnostico.cs`, `EscanearIncompatibilidades`): compara el firmware/Atmosphere **declarado** en la versión instalada del módulo (`ModuloVersion.Firmware` / `ModuloVersion.Atmos`, campos ya existentes del Gist) contra el firmware/Atmosphere **reales** del sistema (firmware de la emuMMC vía NxNandManager, y versión de Atmosphere instalada en la SD) — a diferencia de las Fuentes A/B/C, que solo comparan entre módulos del catálogo entre sí. Si la versión instalada viola el rango declarado, se genera un `HallazgoIncompatibilidad` con `TipoConflicto = "firmware_real"` y se ofrece **ACTUALIZAR** (si `Versiones[0]` del propio módulo sí sería compatible) o **ELIMINAR** (si ni la última versión disponible resuelve el problema) — siempre reutilizando el pipeline de instalación/desinstalación ya existente (`EjecutarInstalacionRapidaAsync`/`EjecutarEliminacionRapidaAsync`), sin ningún mecanismo nuevo de "deshabilitado". Cambios: (1) extraído el evaluador de rangos de versión (`ParseConstraintVersion`, `ViolaConstraint`, `NormalizarVersion`, antes duplicado y privado en `MainWindow.Diagnostico.cs`) a la clase pública reutilizable `Core/VersionConstraintLogic.cs`; (2) `MainWindow.SD.cs` expone dos propiedades tipadas nuevas — `FirmwareEmummcRealDetectado` y `AtmosRealDetectado` — actualizadas junto a los campos de texto de la UI (`InfoSD.TxtFirmwareEmummc`/`InfoSD.TxtAtmosVer`) en todos los puntos donde se refresca el panel derecho (`ComboDrives_SelectionChanged`, `RefrescarPanelInfoSD`, `RefrescarVersionAtmos`, detección async de firmware), para que el diagnóstico no dependa de parsear texto de la UI; (3) `HallazgoIncompatibilidad` gana `HayVersionCompatible` (booleano explícito, solo relevante para `TipoConflicto == "firmware_real"`) y `ModuloAAccionar`/`EsIncompatibleTotal` se extendieron con un `switch` que trata `firmware_real` como "actuar siempre sobre `Modulo`" (nunca `ModuloConflicto`, que en este caso apunta al propio módulo por conveniencia de binding); (4) nueva sección visual **roja** "COMPATIBILIDADES ROTAS" (`SeccionDiagCompatRota`/`ListaDiagCompatRota` en `MainWindow.xaml`, dentro de `ScrollDiag`) calcada de `SeccionDiagIncompat` pero con paleta roja (`#FF4444`/`#3A1515`), separada de la sección amarilla/naranja "CONFLICTOS DE VERSION" (que sigue cubriendo solo A/B/C); ambas secciones comparten el mismo handler `Diagnostico_ClickResolverIncompat`, generalizado para usar `hallazgo.ModuloAAccionar` tanto en ACTUALIZAR como en ELIMINAR (antes ELIMINAR usaba `ModuloConflicto` directamente, correcto para Fuente A pero no aplicable a D).*

*Actualizado: 2025 — rama `feat(incompatibilidades)` — dos ajustes en el panel de Diagnóstico Rápido SD y en la sincronización de SD: (1) `MainWindow.Diagnostico.cs` (`ActualizarDiagnosticoSD`) ahora agrupa las dependencias rotas por **causa principal** en lugar de por módulo afectado: se recorren los módulos instalados con dependencias, se agrupan por el `Id` del módulo causante (`dep.Modulo.Id`) en un diccionario `causasRaiz`, y se genera **una sola** `HallazgoDependencia` por causa con la lista completa de `ModulosAfectados` — evita tarjetas duplicadas cuando varios módulos dependen del mismo módulo roto/desactualizado (ej. Hekate). `HallazgoDependencia` (`Models/Diagnostico/HallazgoDependencia.cs`) se reescribió para reflejar esto: `ModuloCausante` (antes `Modulo`), `Estado` (`EstadoDependencia` de la causa), `ModulosAfectados` (lista), y propiedades computadas `TituloCausa`/`MensajeCausa` que arman el texto de la tarjeta ("HEKATE DESACTUALIZADA", etc.) — consumido por la sección "DEPENDENCIAS ROTAS" en `MainWindow.xaml` (`SeccionDiagDeps`/`ListaDiagDeps`, estilo "causa principal" con icono hexagonal del módulo raíz); (2) corregida una fuga visual donde la barra de búsqueda de catálogo y el botón «VOLVER» aparecían sobre otras vistas (Alertas y Estado, Hub CFW, Asistido) al cambiar de microSD: en `MainWindow.SD.cs` (`ComboDrives_SelectionChanged`), la llamada a `ActualizarFiltrosDelMundo(_mundoSeleccionado.Id)` — que repuebla y muestra `PanelChipsFiltro` (chips + `TxtBusqueda` + `BtnVolverHubDesdeCatalogo`) — ahora está condicionada a `VistaCatalogo.Visibility == Visibility.Visible`, de modo que solo se ejecuta cuando el catálogo es realmente la vista activa.*

*Actualizado: 2025 — rama `feat(optimizacion_de_codigo)` — FASE 1 del plan de migración de `MainWindow` (`docs/MAINWINDOW_MIGRATION_PLAN.md`) completada: extraída a `Core/DiagnosticoCompatibilidadLogic.cs` la lógica pura de escaneo de incompatibilidades (Fuentes A/B/C/D) y de agrupación de dependencias rotas por causa raíz, antes embebida como métodos privados/estáticos en `MainWindow.Diagnostico.cs`. La nueva clase no depende de WPF ni de `ObservableCollection`; recibe colecciones neutras (`IReadOnlyCollection<ModuloConfig>`/`IEnumerable<ModuloConfig>`) como parámetros explícitos. API pública: `EscanearIncompatibilidades(IReadOnlyCollection<ModuloConfig> instalados, string? firmwareEmummcReal, string? atmosReal) : List<HallazgoIncompatibilidad>` y `AgruparDependenciasRotas(IEnumerable<ModuloConfig> instalados, IReadOnlyCollection<ModuloConfig> catalogoCompleto) : List<HallazgoDependencia>` (reutiliza `Core/VersionConstraintLogic.cs` y `Core/AnalizadorDependencias.cs`, sin modificarlos). Consumidores: `MainWindow.Diagnostico.cs` (`ActualizarDiagnosticoSD`, reducido de ~433 a ~164 líneas) y `MainWindow.Navegacion.cs` (`ActualizarResumenesHubCFW`, único otro punto que llamaba al escaneo). Comportamiento, textos y reglas de compatibilidad preservados sin cambios.*

*Actualizado: 2025 — rama `feat(optimizacion_de_codigo)` — FASE 2 del plan de migración de `MainWindow` (`docs/MAINWINDOW_MIGRATION_PLAN.md`) completada: extraída a `Core/DetalleModuloLogic.cs` la máquina de decisión de `ActualizarBotonesDetalle` (qué botones se muestran/ocultan y qué acción representan en la vista de Detalle), antes embebida como método privado en `MainWindow.Detalle.cs`. La nueva clase no depende de WPF ni genera texto de botón: devuelve estado semántico mediante los enums `AccionInstalarDetalle` (`Ninguna`/`InstalarNormal`/`InstalarVersionSeleccionada`/`InstalarVersionSinEliminar`) y `AccionActualizarDetalle` (`Ninguna`/`ActualizarNormal`/`ActualizarAVersion`/`DegradarAVersion`), empaquetados en el record `AccionesDetalleModulo` (flags `Mostrar*`, `EsDegradacion`, `VersionObjetivo`). API pública: `DetalleModuloLogic.DeterminarAcciones(ModuloConfig modulo, ModuloVersion? versionSeleccionada, bool haySd) : AccionesDetalleModulo`. Consumidor: `MainWindow.Detalle.cs` (`ActualizarBotonesDetalle`, reducido de ~100 a ~43 líneas), que traduce el estado semántico a los textos exactos ya existentes (`"INSTALAR EN SD"`, `"ACTUALIZAR A v{version}"`, `"DEGRADAR A v{version}"`, etc.) — la traducción a un futuro sistema multiidioma solo requerirá tocar este punto, sin volver a modificar `Core`. Comportamiento, textos y reglas de instalación/actualización/downgrade preservados sin cambios.*

*Actualizado: 2025 — rama `feat(optimizacion_de_codigo)` — FASE 3 del plan de migración de `MainWindow` (`docs/MAINWINDOW_MIGRATION_PLAN.md`) completada: movidos a `Core/FiltroLogic.cs` el filtro por etiquetas del mundo (`EtiquetasFiltro`) y el ordenamiento por prioridad de `AccionRapida`, antes inline en `RefrescarVistaActual` (`MainWindow.Navegacion.cs`) y parcialmente duplicados conceptualmente con la lógica ya existente en `FiltroLogic`. Nuevos métodos: `FiltroLogic.FiltrarPorEtiquetasMundo(IEnumerable<ModuloConfig>, IReadOnlyCollection<string>? etiquetasBase)` y `FiltroLogic.OrdenarPorPrioridadAccion(IEnumerable<ModuloConfig>)`, ambos sin dependencia de WPF. `RefrescarVistaActual` queda como composición de llamadas a `FiltroLogic` (filtro de mundo ? filtro de chip vía `_cerebro.FiltrarPorEtiqueta` ? filtro de texto vía `_cerebro.FiltrarPorTexto` ? filtro "solo actualizables" [flag de UI, sin cambios] ? orden por `AccionRapida`). Comportamiento, orden y módulos visibles preservados sin cambios.*

*Actualizado: 2025 — rama `fix(Instalacion)` — rediseño de densidad visual de `PanelAsistidoCompletoOverlay`
