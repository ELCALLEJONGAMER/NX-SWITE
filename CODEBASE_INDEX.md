...
*Actualizado: 2025 — rama `fix(Instalacion)` — rediseño de densidad visual de `PanelAsistidoCompletoOverlay` (overlay Asistido Completo/Instalación): (1) overlay ampliado de `700x700` a `780x760` para dar más aire al contenido; (2) la tarjeta SD (`TarjetaSDAsistido`) comprime letra de unidad + nombre + info (capacidad/formato) en una sola línea horizontal (`TxtNombreSDAsistido` + separador `•` + `TxtInfoSDAsistido`), en vez de dos líneas apiladas; (3) el `Slider` continuo de tamaño emuMMC (`SliderGbAsistido`) fue reemplazado por 4 selectores preestablecidos tipo segmented-control (`RadioButton` con `GroupName="EmuMMCAsistido"`): `BtnPreset12`/`BtnPreset24`/`BtnPreset32`/`BtnPreset64`, estilizados con `EstiloPresetEmuMMC` (definido en `Border.Resources` del propio overlay) — `BtnPreset12` lleva el badge "RECOMENDADO" cian y arranca `IsChecked="True"`; (4) `PlantillaModuloRecomendado` (tarjeta de módulo principal en `ListaModulosAsistido`) ya no muestra el mensaje secundario "Instalando (nombre)" (`Nota`/`NotaVisible` de `RecomendadoVM`) — solo el nombre del módulo y el badge de versión fijada; (5) `ScrollModulosAsistido` creció a `Height="190"` (y `380` en modo solo-instalar) para aprovechar el nuevo alto del overlay. Código-behind (`MainWindow.AsistidoCompleto.cs`): `ActualizarPresetEmuMMC(int gb)` reemplaza la lógica del slider — actualiza `_gbEmuMMCAsistido`, `TxtGbValorAsistido.Text` y marca el `RadioButton` correspondiente vía `ObtenerPresetButton(int gb)`; el handler `PresetEmuMMC_Checked` lee `Tag` del `RadioButton` seleccionado. Sin cambios de lógica de negocio (recomendación de tamaño según capacidad de SD, composición de módulos/dependencias, flujo de instalación) — solo presentación.*
