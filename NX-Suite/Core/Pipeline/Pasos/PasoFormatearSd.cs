...
namespace NX_Suite.Core.Pipeline.Pasos
{
    ...
    /// parámetros del JSON y los 3 modos públicos del DiskMaster.
    ///
    /// Parámetros JSON:
    ...
    public class PasoFormatearSd : IPasoPipeline
    {
        ...
        public async Task EjecutarAsync(ContextoPipeline ctx, JsonElement parametros, CancellationToken ct)
        {
            ...
            if (!string.Equals(conf, "SI", StringComparison.OrdinalIgnoreCase))
                throw new Exception("Formateo cancelado por falta de confirmación en el JSON.");
            ...
            else
            {
                ...
                if (discoIndice == -1)
                    throw new Exception("No se pudo identificar el disco físico de la SD para particionar.");
