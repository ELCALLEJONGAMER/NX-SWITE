...
namespace NX_Swite.Core.Configuracion
{
    ...
    ///
    /// Dise±ado para ser tolerante a fallos: si el archivo estß corrupto o
    /// tiene propiedades desconocidas (versi¾n futura/pasada), se cargan los
    /// valores por defecto sin lanzar excepci¾n.
    /// </summary>
    public sealed class GestorPreferencias
    {
        ...
        /// <summary>
        /// Carga las preferencias desde disco. Si el archivo no existe o estß
        ...
        public async Task<PreferenciasUsuario> CargarAsync()
        {
            ...
            {
                // Si el JSON estß corrupto, ignorar y usar defaults
                return new PreferenciasUsuario();
            }
        }
        ...
        /// <summary>
        /// Guarda las preferencias en disco de forma asÝncrona.
        /// Nunca lanza excepci¾n.
        /// </summary>
        public async Task GuardarAsync(PreferenciasUsuario preferencias)
        {
            ...
            catch { /* Fallo silencioso ù no es crÝtico */ }
        }

        // ?? Aplicar al estado estßtico ????????????????????????????????????????
